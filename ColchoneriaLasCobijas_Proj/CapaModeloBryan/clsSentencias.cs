...
namespace CapaModeloBryan
{
    public class clsSentencias
    {
        ...
        //Esta funcion sirve para devolver a la capa controlador los datos que va contener el datagridview de las peticiones
        public OdbcDataAdapter funcLlenarBitacora()
        {
            string Sql = "SELECT B.FECHAHORA_BITACORA, L.USUARIO_LOGIN AS USUARIO, A.NOMBRE_APLICACION, B.DIRECCIONHOST_BITACORA AS DIRECCION_HOST," +
                "B.NOMBREHOST_BITACORA AS NOMBRE_HOST, B.ACCION_BITACORA AS ACCION " +
                " FROM BITACORA B, LOGIN L, APLICACION A WHERE B.FK_IDUSUARIO_BITACORA=L.PK_ID_LOGIN AND B.FK_IDAPLICACION_BITACORA=A.PK_ID_APLICACION" +
                " ORDER BY B.FECHAHORA_BITACORA DESC;";
            OdbcDataAdapter DataTable = new OdbcDataAdapter(Sql, Con.conexion());
            return DataTable;
        }
